...
const ACCENTS = {
  cyan: { icon: 'dark:text-neon-cyan', ring: '', bg: 'dark:bg-neon-cyan/10' },
  purple: { icon: 'dark:text-neon-purple', ring: '', bg: 'dark:bg-neon-purple/10' },
  green: { icon: 'dark:text-neon-green', ring: '', bg: 'dark:bg-neon-green/10' }
};
...
function StatCard({ icon: Icon, label, value, sub, accent = 'cyan' }) {
  ...
  return (
    <div className={`bg-surface dark:bg-abyss-500 border border-line dark:border-abyss-300/40 rounded-2xl p-5 shadow-card transition-shadow ${a.ring}`}>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-ink-900 dark:text-white dark:font-display">{greeting()}, {user?.username}</h1>
          <p className="text-sm text-ink-500 dark:text-abyss-50">Here's what's happening across every client right now.</p>
        </div>
        <Link
          to="/admin/onboard"
          ...
          {[['today', 'Today'], ['14d', '14 days'], ['30d', '30 days']].map(([key, label]) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md transition-colors ${
                range === key
                  ? 'bg-brand-600 dark:bg-neon-cyan/15 text-white dark:text-neon-cyan'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2">
          <CardShell title="Answered vs Rejected" sub={`Call outcomes · ${range === 'today' ? 'today' : `last ${RANGE_DAYS[range]} days`}`} icon={TrendingUp}>
            {trendData.length === 0 ? (
              <p className="text-sm text-ink-400 dark:text-abyss-100 py-16 text-center">No dialed calls in this window yet.</p>
            ) : (
        ...
        <CardShell title="Outcome Mix" sub={overview ? `${overview.totalDialed.toLocaleString()} total calls` : ''} icon={PhoneCall}>
          {outcomeChartData.length === 0 ? (
            <p className="text-sm text-ink-400 dark:text-abyss-100 py-16 text-center">No calls dialed yet.</p>
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <CardShell title="Most Active Clients" sub="By dialed calls" icon={Building2} action={<Link to="/admin" className="text-xs text-brand-600 dark:text-neon-cyan hover:underline">View all</Link>}>
          {!overview?.topTenants?.length ? (
            <p className="text-sm text-ink-400 dark:text-abyss-100 py-6 text-center">No activity yet.</p>
          ) : (
                  ...
                  <div key={t.name} className="flex items-center gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="h-1.5 bg-brand-50 dark:bg-abyss-400/60 rounded-full overflow-hidden">
                        <div className="h-full rounded-full bg-brand-500 dark:bg-neon-cyan" style={{ width: `${Math.max(4, (t.dialed / max) * 100)}%` }} />
                      </div>
                    </div>
                  </div>
                );
          ...
          {overview?.portCapacity && (
            <>
              ...
              <div className="h-2.5 bg-brand-50 dark:bg-abyss-400/60 rounded-full overflow-hidden mb-2">
                <div
                  className={`h-full rounded-full ${portPct > 90 ? 'bg-amber-500' : 'bg-brand-500 dark:bg-neon-cyan'}`}
                  style={{ width: `${portPct}%` }}
                />
              </div>
        ...
        >
          {!overview?.liveCalls?.length ? (
            <p className="text-sm text-ink-400 dark:text-abyss-100 py-6 text-center">No calls in progress right now.</p>
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2">
          <CardShell title="Campaign Control" sub="The work requiring your attention" icon={ShieldAlert}>
            {!overview?.attentionCampaigns?.length ? (
              <p className="text-sm text-ink-400 dark:text-abyss-100 py-6 text-center">Nothing needs attention right now.</p>
            ) : (
        ...
        <CardShell title="Recent Activity" sub="Latest safety & billing events" icon={ShieldAlert}>
          {!overview?.recentActivity?.length ? (
            <p className="text-sm text-ink-400 dark:text-abyss-100 py-6 text-center">No recent activity.</p>
          ) : (
            <div className="space-y-3">
          ...
          columns={[
            {
              ...
              label: 'Tenant',
              render: (t) => (
                <Link to={`/admin/tenants/${t.id}`} className="flex items-center gap-2 text-ink-900 dark:text-white font-medium hover:text-brand-600 dark:hover:text-neon-cyan">
            ...
            { key: 'admin_count', label: 'Admins' },
            {
              ...
              render: (t) => {
                ...
                return p.length ? p.join(', ') : <span className="text-ink-400">none</span>;
              }
            },
            {
              key: 'created_at',
